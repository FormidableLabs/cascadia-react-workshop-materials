  ...
  fields: {
    ...
    purchaseShoppingCart: {
      ...
      resolve: async function(_, { id, name }) {
        if (!id || !name) {
          throw new Error("Cannot purchase shopping cart without an ID or name.");
        }
    ...
    submitContactInfo: {
      ...
      resolve: async function(_, { name, message }) {
        if (!name || !message) {
          throw new Error("Cannot submit contact form without name or message.");
        }
        return { name, message };
      },
    },
